...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = '87143675-9146-43f1-9e41-09a9d12766b0'; // ID de EliGlow Nails
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'EliGlow Nails';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '53088683';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'eliaquiala@gmail.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a EliGlow Nails!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'eliglownails';
};
...
console.log('✅ config-negocio.js listo para EliGlow Nails');
